/**
 * @file MovieGrid.tsx
 * @description Displays a grid of movies with infinite scroll and genre filtering.
 */
...
export default function MovieGrid() {
    ...
    const { search, genreId } = useSearch();
    const apiKey = process.env.NEXT_PUBLIC_TMDB_API_KEY;

    // Reset movie list on new search
    useEffect(() => {
        setMovies([]);
        setPage(1);
    }, [search, genreId]);
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                        {movies.map(movie => (
                            ...
                            >
                                ...
                                <div className="absolute bottom-0 left-0 right-0 bg-black/60 text-white text-sm p-1 truncate text-center">
                                    {movie.title}
                                </div>
                            </Link>
                        ))}
